...
public class Calculator {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Что считаем? ");
        String vvod = s.nextLine();
        System.out.print("Вот что вышло: " + vvod);
        ...
        if (num1 == 0 | num2 == 0) {
            ...
            try {
                ...
                System.out.println("=? \nТы дурак? В школе не проходил это?.");
            }
        } else {
            try {
                result = calculate(num1, num2, oper);
                if (result == 0) {
                    System.out.println("=? \n Ты уверен, что римляне писали так , как ты сейчас меня попросил?");
                    System.exit(0);
                }
                String resultRom = romanSolution(result);
                System.out.println("=" + resultRom + " (" + result + ")");
            } catch (ArrayIndexOutOfBoundsException e) {
                System.out.println("=? \n В этом коде все должно быть положительно, как мое настроение, так и твои числа");
            }
        }
    }
